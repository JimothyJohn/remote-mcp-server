...
class TestSNSTools:
    ...
    def test_allow_mutative_action_only_on_tagged_sns_resource(self):
        ...
        assert (
            message
            == "mutating a resource without the mcp_server_version tag is not allowed"
        )
    ...
    def test_create_topic_override_implementation(self):
        ...
        result = decorated_func(
            name="test-topic.fifo", attributes={}, tags=[], region="us-east-1"
        )
    ...
    def test_is_unsubscribe_allowed_exception(self):
        ...
        # Mock SNS client that raises exception
        mock_sns_client = MagicMock()
        mock_sns_client.get_subscription_attributes.side_effect = Exception(
            "Test exception"
        )
